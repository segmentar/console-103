    ...
    public partial struct MaterialreflectPrimarymodule
    {
        public static MaterialreflectPrimarymodule MaterialreflectModuleAction(Object Object_REFLECT, Boolean answer_RENDER_should)
        {
            ...
            if (answer_RENDER_should is true)
            {
                try
                {
                    Material.Render($"{MaterialreflectCode.RenderPath.Core_Materialreflect_Action}-{nameof(MaterialreflectPrimarymodule)}Action-1-Firstoneten", module.First.Function);

                    Material.Render($"{MaterialreflectCode.RenderPath.Core_Materialreflect_Action}-{nameof(MaterialreflectPrimarymodule)}Action-2-Secondtwotwenty", module.Second.Function);

                    Material.Render($"{MaterialreflectCode.RenderPath.Core_Materialreflect_Action}-{nameof(MaterialreflectPrimarymodule)}Action-3-Thirdthreethirty", module.Third.Function);
